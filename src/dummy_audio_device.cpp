...
DummyAudioDevice::DummyAudioDevice(webrtc::TaskQueueFactory* taskQueueFactory)
    : audio_data(kChannels * kSamplesPerFrame),
      tackQueueFactory_(taskQueueFactory) {}

int32_t DummyAudioDevice::Init() {
  taskQueue_ =
      std::make_unique<rtc::TaskQueue>(tackQueueFactory_->CreateTaskQueue(
          "AudioDevice", webrtc::TaskQueueFactory::Priority::NORMAL));
  task_ = webrtc::RepeatingTaskHandle::Start(taskQueue_->Get(), [this]() {
    ProcessAudio();
    return webrtc::TimeDelta::Millis(kFrameLengthMs);
  });
  initialized_ = true;
  return 0;
}
